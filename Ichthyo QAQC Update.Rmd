...
#this is just for 4/22/2019 through jan 2020
...
fulldata <- allsampcatchspecies %>% 
  ...
  select(-(FieldCommentsExcel)) %>% 
  mutate(CountIndividual = case_when(TotalCountSpecies >= 1 ~ 1, TRUE ~ TotalCountSpecies)) %>%
  relocate(CountIndividual, .after = TotalCountSpecies) %>%
  group_by(event_id, PhysicalDataID, Datetime, Date, Time, Station, YSI, WeatherCode,Tide, MicrocystisVisualRank,
           ...
           MeterSetTime, FlowMeterStart, FlowMeterEnd, FlowMeterSpeed, TowLocation, MeshSize, FieldComments,
          LabComments, SpeciesCode, ScientificName, CommonName, TL,
           FL, LifeStage, LifeStageAbb, TotalCountSpecies, CountIndividual) %>%
  summarise(Count = sum(CountIndividual)) %>% 
  arrange(Datetime)
  

#for notating qaqc comments later - this will be adjusted in future publishing to account for distinction between previously published and newer lab data
noted2 <- noted %>% 
  filter(Datetime < "4/22/2019 9:58")

#adjust the previous publication to match updating lab comments - filter for dates before 4/22/2019
full <- full %>% 
...
samp_catch_physMerge <- fulldata %>%
  ...
  ungroup() %>% 
  select(-c(TotalCountSpecies, CountIndividual)) %>% 
  arrange(Datetime)
...
```{r}
#this step is binding the previous publications data to the newest data for publishing
#ensure that previous data has WY/flowdiff, etc calculated before joining
full_prior <- full %>% 
    mutate(WY = ifelse(Month >9, Year + 1, Year)) %>%
  left_join(wy, by = "WY") %>%
  select(-c(Index, WYType)) %>%
...
fullich <- bind_rows(full_prior, samp_catch_physMerge) %>% 
  arrange(Datetime)
...
```{r}
SamplingQAQC_fill <- read.csv("R_write/IchSamplingQAQC_ 2026-06-08_Notated.csv")
SamplingQAQC_fill_s <- SamplingQAQC_fill %>%
  select(c(event_id, Flag_SAMP, Comment_SAMP, Flag_LAB, Comment_LAB))
#bring in previous qaqc notated file as well to bind with newest data to ensure you have the full dataset
...
SamplingQAQC_fill_sfinal <- bind_rows(SamplingQAQC_fill_s2, SamplingQAQC_fill_s)
...
samp_catch_phys <- left_join(fullich, inundation4)
```
...
samp_catch_phys$FlowMeterSpeed <- str_replace_all(samp_catch_phys$FlowMeterSpeed, "^Reg$", "Regular")
samp3$FlowMeterSpeed <- str_replace_all(samp3$FlowMeterSpeed, "^Reg$", "Regular")
...
22. Merge data with lab/sampling QAQC 
  * Replace NA with 0 (0 = no comment/no flag)
```{r}
...
Flow.outlier.STTD$FlowMeterSpeed <- str_replace_all(Flow.outlier.STTD$FlowMeterSpeed, "^Reg$", "Regular")
